...
def phase_private_config(config: dict):
    ...
    if config["email_address"]:
        email_block = (
            f"# Email alerts\n"
            f"email_address: '{config['email_address']}'\n"
            f"email_pwd: '{config['email_pwd']}'\n"
            f"email_server: '{config['email_server']}'\n"
        )
    else:
        email_block = (
    ...
    content = (
        f"# private_config.yaml\n"
        f"# Generated by sysinit/setup_live_trading.py\n"
        f"# This file is excluded from git — keep it safe.\n"
        f"\n"
        f"# ── REQUIRED ─────────────────────────────────────────────────────────\n"
        f"# Your IBKR account number.\n"
        f"# Paper accounts start with DU (e.g. DU1234567).\n"
        f"# Live accounts start with U  (e.g. U1234567).\n"
        f"broker_account: '{config['broker_account']}'\n"
        f"\n"
        f"# Directory for Parquet time-series price data.\n"
        f"parquet_store: '{config['parquet_store']}'\n"
        f"\n"
        f"# ── IB GATEWAY ───────────────────────────────────────────────────────\n"
        f"ib_ipaddress: '{config['ib_ipaddress']}'\n"
        ...
        f"mongo_db: 'production'\n"
        f"\n"
        f"# ── BACKUPS ──────────────────────────────────────────────────────────\n"
        f"offsystem_backup_directory: '{config['backup_dir']}'\n"
        f"\n"
        f"# ── TRADING ──────────────────────────────────────────────────────────\n"
        f"# Base currency of your trading account.\n"
